...
class deformable_attention(nn.Module):
    def __init__(self, in_ch, out_ch, k):
        ...
        self.last_conv = nn.Conv2d(out_ch, out_ch, 1, 1, 0, bias=True)
    ...
    def forward(self, input):
        ...
        return self.last_conv(feat) + input




class weighted_deformable_attention(nn.Module):
    def __init__(self, in_ch, out_ch, k):
        super().__init__()
        self.conv = nn.Conv2d(in_ch, out_ch, 1, 1, 0, bias=True)
        self.k = k
        ...
        self.offset_conv = nn.ModuleList(offset_list)
        self.weight_conv = nn.Sequential(nn.Conv2d(in_ch, k, 1, 1, 0, bias=True), nn.Softmax(1))
        self.alpha = nn.Parameter(torch.tensor([0]).float())
    ...
    def forward(self, input):
        ...
        return input + feat * self.alpha


class efficient_deformable_attention(nn.Module):
    def __init__(self, in_ch, out_ch, k):
        super().__init__()
        self.k = k
        self.out_ch = out_ch
        self.offset_conv = nn.Conv2d(in_ch, 3 * k, 1, 1, 0, bias=True)
    ...
    def forward(self, input):
        b, c, h, w = input.size()
        proj_feat = self.offset_conv(input)
        offsetweights = torch.repeat_interleave(self.softmax(proj_feat[ :, 2 * self.k : 3 * self.k, :, :]), self.out_ch, 1)
        feats = []
        for i in range(self.k):
            flow = proj_feat[ :, 2*i: 2*i + 2, :, :]
            flow = flow.permute(0, 2, 3, 1)
            grid_y, grid_x = torch.meshgrid(torch.arange(0, h), torch.arange(0, w))
            grid = torch.stack((grid_x, grid_y), 2).float()
            grid.requires_grad = False
            grid = grid.type_as(input)
            vgrid = grid + flow
            vgrid_x = 2.0 * vgrid[:, :, :, 0] / max(w - 1, 1) - 1.0
            vgrid_y = 2.0 * vgrid[:, :, :, 1] / max(h - 1, 1) - 1.0
            vgrid_scaled = torch.stack((vgrid_x, vgrid_y), dim=3)
            feat = F.grid_sample(input, vgrid_scaled, mode='bilinear', padding_mode='zeros')
            feats.append(feat)
...
class deformable_attention_single(nn.Module):
    def __init__(self, in_ch, out_ch, k):
        super().__init__()
        self.conv = nn.Conv2d(in_ch, out_ch, 1, 1, 0, bias=True)
        self.k = k
