...
class MAB(nn.Module):
    def __init__(self, n_feats, reduction_factor=4, distillation_rate=0.25):
        ...
        self.conv1 = conv_block(n_feats//reduction_factor, n_feats//reduction_factor,3,dilation=1,act_type='lrelu')     #普通卷积
        self.conv2 = conv_block(n_feats // reduction_factor, n_feats//reduction_factor, 3,dilation=2,act_type='lrelu')  #空洞卷积
...
class MAFFSRN(nn.Module):
    def __init__(self,args):
        ...
        # define tail module
        n_colors = 3
        tail = Tail(n_colors, scale, n_feats, kernel_size, wn)
...
if __name__ == '__main__':
    ...
    model=MAFFSRN({"scale":4,"n_FFG":4})
    # print(get_parameter_number(model))
    from torchsummary import summary
    summary(model, (3, 320, 180),depth=8,verbose=1,col_names=["input_size", "output_size","kernel_size", "num_params","mult_adds"])
