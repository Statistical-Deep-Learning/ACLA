...
class Channel_Spatial_Attention_Module(nn.Module):
    def __init__(self, initial_gamma_CSAM, fix_gamma_CSAM=False):
        super(Channel_Spatial_Attention_Module, self).__init__()
        self.conv = nn.Conv3d(1, 1, 3, 1, 1)
        self.sigmoid = nn.Sigmoid()
        self.gamma = nn.Parameter(torch.tensor([initial_gamma_CSAM]).float(), requires_grad=not fix_gamma_CSAM)
...
class RCAN_HAN(nn.Module):
    def __init__(self, args, conv=common.default_conv):
        ...
        self.csa = Channel_Spatial_Attention_Module(initial_gamma_CSAM=args.initial_gamma_CSAM, fix_gamma_CSAM=args.fix_gamma_CSAM)
        self.la = LAM_Module(n_feats, initial_gamma=args.initial_gamma_LAM, fix_gamma=args.fix_gamma_LAM, initial_beta=args.initial_beta_LAM)
