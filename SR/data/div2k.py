...
class DIV2K(srdata.SRData):
    ...
    def __init__(self, args, train=True):
        ...
        # test_every: do test per every N batches, 每 1000 个循环测一次
        # n_train: number of training set
        # 每个batch（16）一次循环，800张图的测试集遍历完一次要50个循环，循环1000次，就是一个 epoch 16000 张图，相当于重复20遍
        self.repeat = args.test_every // (args.n_train // args.batch_size)  # 1000 // (800 // 16) = 1000 // 50iter = 20
        print('The value of repeat' + str(self.repeat))
        print()
    def _scan(self):
        """
        Return:
