...
class HAN(nn.Module):
    ...
    def forward(self, x):
        ...
        for name, midlayer in self.body._modules.items():
            x = midlayer(x)
            if name == '0':
                LA_input = x.unsqueeze(1)
            else:
                if name != str(self.n_resgroups):
                    LA_input = torch.cat([x.unsqueeze(1), LA_input], 1)

        feature_LA = self.LA(LA_input)  # b, n * c, h, w
        feature_CSA = self.CSA(x)  # # b, c, h, w

        x = self.tail(body_input + feature_CSA + feature_LA)
        x = self.add_mean(x)
