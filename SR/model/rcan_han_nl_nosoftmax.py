...
class LAM_Module(nn.Module):
    ...
    def forward(self, x):
        ...
        attention_map = attention_map / (N * C * height * width)

        # attention_map = self.softmax(attention_map)

        proj_value = x.view(m_batchsize, N, -1)

        attention_out = torch.bmm(attention_map, proj_value)

        attention_out = self.nl_conv(attention_out.view(m_batchsize, -1, height, width))

        out = attention_out.view(m_batchsize, N, C, height, width) + x
...
class RCAN_HAN(nn.Module):
    def __init__(self, args, conv=common.default_conv):
        ...
        self.last = nn.Conv2d(n_feats * 2, n_feats, 1, 1, 0)
